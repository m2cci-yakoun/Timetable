...
def rech_s():
    list1.delete(0,END)
    for row in searchS(service.get()):
        list1.insert(END,row)

def rech_d():
    list1.delete(0,END)
    for row in show_day_work(date.get()):
        list1.insert(END,row)
...
e1['values']=("",'TRANS','DET','MANOEUVRE','TIMONERIE','SECURITE','MACHINE','ELECTRICITE','ARTILLERIE')
...
b1=Button(window,text="rechercher par date",width=25,command=rech_d)
...
boudara=Label(window,text='PHM RAIS BARGACH')
